...
import com.unbank.robotspider.filter.title.TitleBySqlFilter;
...
public class TitlePaser extends BasePaser {
	...
	public boolean fillCrawlTitle(NewsInfoMiddleWare articleInfo,
			Document document) {
		String title = null;
		if (articleInfo.getWebsiteId() > Values.v.WEBSITEID) {
			try {
				TitleFilter baseFilter = new TitleBySqlFilter(articleInfo);
				title = baseFilter.getTitle(document,
						articleInfo.getCrawlTitle());
			} catch (Exception e) {
			...
			title = TitleFilterLocator.getInstance()
