...
public class NewsDatePaser {
	...
	public boolean analyzeWebDate(Document document) {
		Date webDate = getDate(document);
		if (webDate == null) {
			return false;
		}
		Date today = getyyyyMMddTimeString(0);
		Date yestoday = getyyyyMMddTimeString(-7);
		if (webDate.before(today) && webDate.after(yestoday)) {
			return true;
		} else {
			return false;
		}
	...
	private Date getyyyyMMddTimeString(int num) {
