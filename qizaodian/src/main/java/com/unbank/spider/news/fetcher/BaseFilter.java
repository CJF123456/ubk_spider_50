package com.unbank.spider.news.fetcher;
...
import com.unbank.spider.tool.StringUtil;
...
public class BaseFilter implements Filter {
	...
	public void formatElements(Element maxTextElement) {
		...
		for (Element element : allElements) {
			...
			if (element.tagName().equals("br")) {
			...
			} else if (!(element.tagName().equals("img") || element.tagName().equals("table"))) {
				// 去除属性
				removeAttr(element);
				// 更换标签
				if (element.tagName().equals("p") || element.tagName().equals("div")) {
					...
					if (element.tagName().equals("br") || element.tagName().equals("img")
							|| element.tagName().equals("table") || element.tagName().equals("th")
							|| element.tagName().equals("tr") || element.tagName().equals("td")
							|| element.tagName().equals("tbody") || element.tagName().equals("strong")
	...
	/*public void saveImage(Element maxTextElement, String url) {
		...
		for (Element element : elements) {
			...
			if (imgUrl != null && (!imgUrl.trim().isEmpty())) {
				logger.info("网址         " + url + "    图片地址      " + imgSrc + "       新图片网址    " + imgUrl);
				...
				logger.info("图片下载出错网址         " + url + "    图片地址      " + imgSrc + "       新图片网址      " + imgUrl);
				element.remove();
			}
		}
...
*/
	...
	public String removeStockCode(String content) {
		String stockCodes[] = new String[] { "<!--.[^-]*(?=-->)-->", "(?is)<!--.*?-->", "\\(\\d{6},\\)",
				"\\(\\d{1,6}[\\.|,|，|；|;|-][ |(&nbsp;)]{0,10}\\w{2}\\)",
				"\\(\\d{1,6}\\.\\w{2,4}[，|,|;|；][\\u4e00-\\u9fa5]{2,10}\\)",
				"\\(\\d{1,6}\\.\\w{2,4}/\\d{1,6}\\.\\w{2,4}\\)", "\\(\\d{1,6}\\.\\w{2,4};\\d{1,6}\\.\\w{2,4}\\)",
				"\\(\\d{1,6}\\.\\w{2,4}；\\d{1,6}\\.\\w{2,4}\\)", "\\(\\d{1,6}\\.\\w{2,4}、\\d{1,6}\\.\\w{2,4}\\)",
				"\\(\\d{1,6}\\.\\w{2,4},\\d{1,6}\\.\\w{2,4}\\)", "\\(\\d{1,6}\\.\\w{2,4}，\\d{1,6}\\.\\w{2,4}\\)",
	...
	public String removeNoNeedChars(String content) {
		...
		String[] spechars = new String[] { "【免责声明", "【版权声明", "【重点推荐", "【延伸阅读", "【推荐阅读", "【相关阅读", "免责声明", "版权声明",
				"【更多详情", "【相关专题" };
	...
	public String replaceNoNeedChars(String content) {
		String spechars[] = new String[] { "我有话说", "欢迎发表评论", "发表评论", "收藏本文", "微博推荐", "(行情研报)", "[微博]", "(微博)", "(财苑)",
				"返回列表", "加入收藏", "打印本页", "打印本稿", "新闻订阅", "分享到", "【打印】", "网站论坛", "字体：大 中 小", "字体:大 中 小", "字体：", "字体:",
				"推荐朋友", "关闭窗口", "关闭", "慧聪资讯手机客户端下载", "(行情，问诊)", "(行情,问诊)", "(博客,微博)", "(微信)", "(市值重估)",
				"[最新消息 价格 户型 点评]", "[简介 最新动态]", "[简介最新动态]", "[最新消息价格户型点评]", "(楼盘)", "(点击查看最新人物消息)", "( 详情 图库 团购 点评 ) ",
				"(CNFIN.COM/XINHUA08.COM)--", "(CNFIN.COM / XINHUA08.COM)--", "(CNFIN.COM&nbsp;/&nbsp;XINHUA08.COM)--",
				"(看跌期权)", "(放心保)", "(查询信托产品)", "(楼盘资料)", "(<strong>点击进入徐工网上购机页面</strong>)", "点击浏览全文", "返回首页",
				"更多精彩，请登录“人民微博”参与互动", "《》：", "(滚动资讯)", "(完)", "(中国电子商务研究中心讯)", "()" };
		...
		while (true) {
			...
			for (int i = 0; i < childElements.size() - 1; i++) {
				...
				for (int j = i + 1; j < childElements.size(); j++) {
					// 如果包含标题
					int iLength = getElemnetTextLength(title, childElements.get(i).text());
					int jLength = getElemnetTextLength(title, childElements.get(j).text());
					if (iLength < jLength) {
	...
	public Element getMaxTextElement(String url, String title, Element parentElement) {
	...
	private Element getPossibleParent(Element temp) {
		while (true) {
			if (temp.tagName().equals("p") || temp.tagName().equals("div") || temp.tagName().equals("td")) {
	...
	private Element getPnodeParent(Element temp) {
		if (temp.siblingElements().size() == 0) {
			...
			for (Element element : elements) {
				...
				} else if ((!element.tagName().equals("table")) && (!element.tagName().equals("img"))
						&& (!element.tagName().equals("strong")) && (!element.tagName().equals("b"))
						&& (!element.tagName().equals("br")) && (!element.tagName().equals("center"))) {
	...
	private void removeNoImageElemnet(Element element) {
		...
		if (element.attr("style").contains("text-align:  center")
				|| element.attr("style").contains("text-align: center") || element.attr("align").contains("center")
	...
	private Element getTdParentNode(Element temp) {
		...
		if (elements.size() == 0) {
			...
			for (Element element : elements) {
				if ((!element.tagName().equals(temp.tagName())) && (!element.tagName().equals("p"))
	...
	public void removeWaste(Element element) {
		if (element != null && element.attr("style").contains("display:none;")) {
			element.remove();
		} else if (element.tagName().equals("script") || element.tagName().equals("style")
				|| element.tagName().equals("textarea") || element.tagName().equals("select")) {
			element.remove();
		} else if (element.tagName().equals("li") || element.tagName().equals("dl") || element.tagName().equals("dt")
	...
	private void removeLinkNode(Element element) {
		...
		Elements elements = element.children();
		for (Element element2 : elements) {
			if (element2.tagName().equals("a") || element2.tagName().equals("br") || element2.tagName().equals("img")
				...
				if (elements2.size() == 0) {
					// 如果没有兄弟节点
					if (element2.children().size() == 1 && element2.children().first().tagName().equals("a")) {
			...
			for (String reg : regList) {
				...
				while (matcher.find()) {
					...
					content = content.replace(str,
							str.replace("(", "<protecteChars>").replace(")", "</protecteChars>"));
				}
			}
		}
		return content;
	...
	public void removeAdvertiseText(Element maxTextElement) {
		...
		for (Element element : elements) {
			String advertiseTexts[] = new String[] { "查看", "||", "相关评论", "相关专题", "重点推荐", "延伸阅读", "推荐阅读", "相关报道", "商业专栏",
					"证券要闻", "行业动态", "公司新闻", "相关新闻", "往期回顾", "今日消息", "机构策略", "原标题", "相关阅读", "更多详情", "免责声明", "版权声明",
					"相关链接", "机构研究", "个股点评", "行业新闻", "公司动态" };
